...
// <head> (index.html / about.html / service.html), which runs before
// the page paints so there's no visible jump on mobile or desktop.
...
// No-ops safely on pages (like About/Service) that don't have this hero.
...
if (window.gsap) {
  ...
  if (!prefersReducedMotion) {
    ...
    // Service hero background parallax (scroll only)
    const svcHeroBg = document.querySelector('.svc-hero-bg');
    const svcHeroSection = document.querySelector('.svc-hero');
    if (svcHeroBg && svcHeroSection) {
      gsap.to(svcHeroBg, {
        yPercent: 8,
        ease: 'none',
        scrollTrigger: {
          trigger: svcHeroSection,
          start: 'top top',
          end: 'bottom top',
          scrub: true
        }
    ...
    // Service), this timeline simply animates nothing and is a safe no-op.
    ...
    gsap.utils.toArray('.stats-row .num, .mini-stat .num, .stat-card .big, .team-stat-card .num, .revenue-figure .num, .svc-hero-stat .num, .svc-image-stat .num').forEach((el) => {
